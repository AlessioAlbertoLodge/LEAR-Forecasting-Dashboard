...
# ----------------------------
# Choose "today" (default = real today)
# ----------------------------
now_real = pd.Timestamp.now(tz=tz)
default_today_local = now_real.normalize().date()

use_custom_today = st.sidebar.toggle("Override 'today'", value=False)
if use_custom_today:
    chosen_today_date = st.sidebar.date_input("Chosen 'today' (local)", value=default_today_local)
else:
    chosen_today_date = default_today_local

# anchor "today" timestamp in display tz
today_local = pd.Timestamp(chosen_today_date, tz=tz)

st.sidebar.caption(f"Effective 'today': {today_local.date()} ({tz})")
...
st.caption(
    "LEAR is trained on an hourly UTC-naive index to avoid DST ambiguity; "
    "plots can still be shown in your chosen timezone. "
    "For Day X forecasting, LEAR trains only on data strictly before Day X (by construction)."
)


# Forecast targets defined from chosen today
day_x = today_local + pd.Timedelta(days=1)   # "tomorrow" relative to chosen today
day_x1 = today_local + pd.Timedelta(days=2)  # "day after tomorrow" relative to chosen today
...
def determine_start_fetch_native(eic: str, tz: str, end_fetch: pd.Timestamp, months_back: int) -> tuple[pd.Timestamp, int]:
    # Use a small recent window around chosen today
    s0 = today_local - pd.Timedelta(days=2)
    e0 = today_local + pd.Timedelta(days=1)
...
# ---------- fetch window ----------
# We only need to fetch up to chosen_today + 1 day (exclusive) to have:
# - training: everything < chosen_today
# - plus day_x (chosen_today+1) actuals when ENTSO-E publishes them (often available)
# Need the full Day X hours present in df for LEAR to build Xtest
end_fetch = day_x + pd.Timedelta(days=1)  # exclusive end (covers full Day X)
# If you still want to *try* Day X+1 availability, you can use:
# end_fetch = day_x1 + pd.Timedelta(days=1)
...
# Build UTC-naive LEAR dataframe
lear_df_utc_all = build_lear_hourly_df_from_prices(
    prices_1h,
    tz_for_display=tz,
    force_freq="1h",
    index_tz="UTC",
...
# Keep full df (it must include Day X rows for Xtest).
# LEAR itself enforces "training uses only data before Day X" via next_day_date.
lear_df_utc = lear_df_utc_all.copy()


# Targets (UTC-naive) for LEAR forecasting calls
...
# ---- Patch Day X if 1–2 hours missing (common when day-ahead publication is incomplete)
# NOTE: This patch is only for the test-day coverage inside LEAR's internal split.
lear_df_utc, patch_info_x = ensure_full_target_day_for_lear(
    lear_df_utc,
...
diag_x = validate_lear_coverage(lear_df_utc, next_day_x_utc_naive, require_full_24h=True)
if not diag_x.get("ok", False):
    st.error("LEAR input data is not sufficient to forecast Day X (tomorrow).")
    st.write(diag_x)
    st.stop()

# ---- Day X+1: if 0 hours available -> skip forecast entirely
diag_x1 = validate_lear_coverage(lear_df_utc, next_day_x1_utc_naive, require_full_24h=False)
if diag_x1.get("n_target_rows", 0) == 0:
    st.info("ENTSO-E has not published any hours for Day X+1 yet. Skipping Day X+1 forecast to avoid empty Xtest.")
    with st.expander("Day X+1 diagnostics"):
        st.write(diag_x1)
    do_forecast_x1 = False
else:
    diag_x1_full = validate_lear_coverage(lear_df_utc, next_day_x1_utc_naive, require_full_24h=True)
    if not diag_x1_full.get("ok", False):
        st.info("Day X+1 is not fully available (not 24/24). Skipping Day X+1 forecast to avoid empty/partial Xtest.")
        with st.expander("Day X+1 full-day diagnostics"):
            st.write(diag_x1_full)
        do_forecast_x1 = False
    else:
        do_forecast_x1 = True

# calibration window in days (based on training-only df)
history_days = int((lear_df_utc.index.max() - lear_df_utc.index.min()).days)
...
c2.metric("Training start (UTC)", f"{lear_df_utc.index.min().date()}")
c3.metric("Training end (UTC)", f"{lear_df_utc.index.max().date()}")
...
# Actual series for Day X (if available in the training df after patch)
...
# ---------- Day X+1 plot ----------
st.subheader("Day X+1 (day after tomorrow)")
idx_x1 = pred_index_for_day(day_x1, display_tz=tz)

if pred_day_x1 is None:
    st.info("Forecast not produced because ENTSO-E has not published Day X+1 yet (0/24 hours).")
    df_plot_x1 = pd.DataFrame({"timestamp": idx_x1, "forecast": [None] * len(idx_x1)})
else:
    pred_vec_x1 = flatten_lear_prediction(pred_day_x1)
    if len(pred_vec_x1) != 24:
        st.info(f"Skipping Day X+1 plot: prediction length {len(pred_vec_x1)} (expected 24).")
        df_plot_x1 = pd.DataFrame({"timestamp": idx_x1, "forecast": [None] * len(idx_x1)})
    else:
...
with st.expander("Hourly series used for LEAR (UTC-naive index, TRAINING ONLY)"):
